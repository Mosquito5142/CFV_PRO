        ...
        <div class="card-body p-4">
          ...
          <div class="space-y-2 mb-3">
            ...
            <div class="flex gap-2">
              ...
              <select v-model="selectedSet" class="select select-bordered select-xs flex-1">
                <option value="">All Sets</option>
                <option v-for="set in sets" :key="set">{{ set }}</option>
              </select>
            </div>
            ...
            <div class="flex gap-2">
              ...
              <button @click="resetFilters" class="btn btn-xs btn-ghost text-error">Reset</button>
            </div>
      ...
      <div class="w-full lg:w-1/2 card bg-base-200 shadow-xl" id="Deck">
        <div class="card-body p-4">
          ...
          <div class="flex gap-2 mb-3">
            <button @click="sortDeck" class="btn btn-xs btn-outline btn-primary flex-1">↕️ Sort by Grade</button>
            <button @click="sortDeckByName" class="btn btn-xs btn-outline flex-1">🔤 Sort by Name</button>
          </div>
            ...
            <div v-for="group in groupedDeck" :key="group.card.id"
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex gap-1">
                  ...
                  <span v-if="group.card.triggereffect" class="badge badge-xs badge-warning">{{ group.card.triggereffect
                    }}</span>
                </div>
              </div>
              <div class="flex items-center gap-1">
                <button @click="removeOneCard(group.card)" class="btn btn-circle btn-xs btn-ghost text-error">−</button>
                ...
                <button @click="addCardToDeck(group.card)" :disabled="group.count >= 4 || !canAddCard(group.card)"
                  class="btn btn-circle btn-xs btn-ghost text-success">+</button>
              </div>
            </div>
          ...
          <div class="mt-3">
            <button @click="preparePreview" :disabled="deck.length === 0" class="btn btn-success w-full">
        ...
        <div v-if="previewCard">
          ...
          <div class="modal-action">
            <button @click="addCardToDeck(previewCard); closePreview()"
              ...
              class="btn btn-primary btn-sm">Add to Deck</button>
            <form method="dialog">
    ...
    <dialog id="preview_modal" class="modal">
      <div class="modal-box max-w-4xl">
        ...
        <h3 class="font-bold text-lg mb-4">
          <span class="bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">🎴 Deck Preview</span>
        </h3>
        ...
        <div class="grid grid-cols-8 sm:grid-cols-10 gap-1 mb-4 max-h-[50vh] overflow-y-auto">
...
onMounted(() => {
  clans.value = [...new Set(allCards.map(card => card.clan))].sort();
  grades.value = [...new Set(allCards.map(card => card.grade))].sort();
  sets.value = [...new Set(allCards.flatMap(card => card.sets || []))].sort();
  filterCards();
});
...
const filterCards = () => {
  ...
  if (selectedClan.value) filtered = filtered.filter(card => card.clan === selectedClan.value);
  if (selectedSet.value) filtered = filtered.filter(card => card.sets?.includes(selectedSet.value));
  if (selectedGrade.value) filtered = filtered.filter(card => card.grade === selectedGrade.value);
  if (selectedTrigger.value) {
    if (selectedTrigger.value === 'None') {
      filtered = filtered.filter(card => !card.triggereffect);
    } else {
      filtered = filtered.filter(card => card.triggereffect?.includes(selectedTrigger.value));
    }
  }
  if (searchQuery.value) filtered = filtered.filter(card => card.name.toLowerCase().includes(searchQuery.value.toLowerCase()));
...
const getCardCountInDeck = (cardName) => deck.value.filter(c => c.name === cardName).length;
...
const canAddCard = (card) => {
  ...
  const healCount = deck.value.filter(c => c.triggereffect?.includes('Heal')).length;
  ...
  if (card.triggereffect?.includes('Heal') && healCount >= 4) return false;
  return true;
};

const addCardToDeck = (card) => {
  if (!canAddCard(card)) {
    if (getCardCountInDeck(card.name) >= 4) showToast(`${card.name} is at max (4)`, 'warning');
    else if (deck.value.filter(c => c.cardtype === 'Trigger Unit').length >= 16) showToast('Trigger limit (16)', 'error');
    else if (deck.value.filter(c => c.triggereffect?.includes('Heal')).length >= 4) showToast('Heal limit (4)', 'error');
    else showToast('Deck full (50)', 'error');
    return;
  }
...
const sortDeck = () => {
  deck.value.sort((a, b) => a.grade === b.grade ? a.name.localeCompare(b.name) : a.grade - b.grade);
...
const groupedDeck = computed(() => {
  ...
  return Object.values(groups).sort((a, b) => a.card.grade === b.card.grade ? a.card.name.localeCompare(b.card.name) : a.card.grade - b.card.grade);
});

const sortedDeckForPreview = computed(() => [...deck.value].sort((a, b) => a.grade === b.grade ? a.name.localeCompare(b.name) : a.grade - b.grade));

const deckValidation = computed(() => {
  const msgs = [];
  if (nonGUnitCount.value > 50) msgs.push({ type: 'error', text: 'Deck exceeds 50 cards!' });
  if (triggerUnitCount.value > 16) msgs.push({ type: 'error', text: 'Too many triggers!' });
  if (nonGUnitCount.value > 0 && nonGUnitCount.value < 50) msgs.push({ type: 'warning', text: `Need ${50 - nonGUnitCount.value} more cards` });
  if (triggerUnitCount.value < 16 && nonGUnitCount.value > 0) msgs.push({ type: 'warning', text: `Need ${16 - triggerUnitCount.value} more triggers` });
  return msgs;
});
...
const closePreview = () => document.getElementById('card_preview_modal').close();
const preparePreview = () => document.getElementById('preview_modal').showModal();

const uploadDeck = () => {
  isLoading.value = true;
  axios.post('/api/macros/s/AKfycbwxQSc_FdDlkU81payets_7BpBtqs61yDI_yLcIe_of7sArG3fhg-UZps8VIW3s486FvA/exec', {
    name: deckName.value,
...
const scrollToDeck = () => document.getElementById('Deck')?.scrollIntoView({ behavior: 'smooth' });
...
watch([selectedClan, selectedSet, selectedGrade, selectedTrigger, searchQuery], filterCards);
</script>
